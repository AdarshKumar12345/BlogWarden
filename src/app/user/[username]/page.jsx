...
const UserPosts = ({posts}) => {
    ...
    return <div className="flex flex-col gap-4 w-10/12 pt-10 mx-auto">
        ...
        : (
            posts.map((post)=> {
            return <Link key={post.title} className="flex items-center gap-5 bg-zinc-800/40 hover:bg-zinc-800/20 transition-all duration-200 hover:scale-[1.03] rounded px-2 py-2 w-full mx-auto" href={`/blog/${post.slug}`}>
                <Image className="w-36 h-20" src={post.thumbnail} width={100} height={60} alt={post.title} />
                <div>
